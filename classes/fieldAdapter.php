...
class fieldAdapterPps {
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', PPS_LANG_CODE) : langPps::_($notSelected);
			}
    ...
    static public function getStates($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if($notSelected) {
				...
				$options[ $notSelected ][0] = array('name' => langPps::_( $notSelectedLabel ), 'country_id' => NULL);
			}
    ...
    static function productFieldCategories($field){
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsPps'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):langPps::_('Select All', PPS_LANG_CODE);
            foreach($categories as $c) {
